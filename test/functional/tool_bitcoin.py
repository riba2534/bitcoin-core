...
class ToolBitcoinTest(BitcoinTestFramework):
    ...
    def test_install_dirs(self):
        ...
        # bitcoind reads and writes its data directory before it handles
        # -version, so give it one in the test directory. The default data
        # directory is not writable on every machine, and the data directory of
        # the node above can contain an ipcbind= line, which would make the
        # wrapper look for bitcoin-node instead of bitcoind.
        datadir = Path(self.options.tmpdir) / "install-datadir"
        datadir.mkdir()
        ...
        def run_wrapper(prefix_name, bin_dir, libexec_dir):
            ...
            return subprocess.run([wrapper, "node", f"-datadir={datadir}", "-version"], capture_output=True, timeout=60)
